package lianxi.前世的名字;

import lianxi.前世的名字.Datas;
import lianxi.前世的名字.information;
...
//            用户输入数据
            System.out.println("请注册，注册需要输入用户名，密码，生肖，月份，星座");
            String sname = sr.next();
            int psd = sr.nextInt();
            String animal = sr.next();
            String month = sr.next();
            String star = sr.next();
...
//                System.out.println("不继续输入输入除了1的数字");
        ...
        //用户注册
        public static int register(String sname,int psd, String animal,String month,String star,information[] uarr){
            // 创建一个用户对象
